...
import Image from "next/i  const { isVariantAvailable, getVariantQuantity } = useInventoryContext();

  // Sprawdź czy produkt ma wiele wariantów (więcej niż 1 opcję do wyboru)
  // Dla uproszczenia - jeśli firstVariant ma różne opcje w tytule to wymagaj wyboru
  const requiresVariantSelection = product.firstVariant?.title?.includes('/') || false;
...
export default function ProductCard({ product, className }: ProductCardProps) {
  const formatPrice = (price: ProductPreview['price']) => {
    ...
    return formatter.format(price.amount / 100); // Assuming price is in cents
  };
  ...
  const { isVariantAvailable, getVariantQuantity, isLoading, error, inventory } = useInventoryContext();
  ...
  console.log('ProductCard Debug:', {
    productTitle: product.title,
    firstVariantId: product.firstVariant?.id,
    isInStock,
    inventoryQuantity,
    isLoading,
    error,
    inventoryData: product.firstVariant ? inventory[product.firstVariant.id] : 'No variant'
  });
